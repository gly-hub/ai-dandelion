...
func TestGetSystemClientUsesConnectionPoolForEveryRequest(t *testing.T) {
	...
	controller := NewSystemServerController(manager)
	const requests = 128
	...
	for range requests {
		...
		go func() {
			defer wg.Done()
			if _, err := controller.getSystemClient(context.Background()); err != nil {
